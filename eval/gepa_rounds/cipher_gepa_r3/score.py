#!/usr/bin/env python3
"""Score GEPA round 3: read predictions, compute EM per program."""
import json, os
...
for b in batches:
    ...
    print(f"[{pi}] {b.get('desc','')} | EM={em:.0%} ({matches}/{n}) miss={missing} "
...
print(f"Best: prog_{best['program_idx']} fitness={best['fitness']:.3f} EM={best['em']:.0%}")
print(f"  {best['desc']}")
